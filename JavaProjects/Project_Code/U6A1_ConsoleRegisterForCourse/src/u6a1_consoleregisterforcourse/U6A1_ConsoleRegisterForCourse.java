...
public class U6A1_ConsoleRegisterForCourse {
    ...
    public static int ValidateChoice(int choice, int firstChoice, int secondChoice, int thirdChoice, int totalCredit) {
        ...
        int result = 0; //intialized variable to hold check results
        
      if(choice<1 || choice>7){ //validate input
          result = -1;
      }
      else if(choice==firstChoice || choice == secondChoice || choice == thirdChoice){ // check for duplicates
          result = -2;
       }
      
      else if(totalCredit >= 9){// checks total credits registered
         result = -3;
      }
      return result;
